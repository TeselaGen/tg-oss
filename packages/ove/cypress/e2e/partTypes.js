describe("partTypes", function () {
  ...
  it("should be able to make a part from feature", () => {
    ...
    cy.contains(".bp5-menu-item", "Make a Part from Feature").click();
    cy.get(".bp5-menu").should("not.exist");
    cy.contains(".veRowViewPart title", "test feature").trigger("contextmenu", {
      force: true
    });
    cy.contains(".bp5-menu-item", "Edit Part", { timeout: 15000 }).click();
    cy.get(".bp5-dialog-body").contains(".tg-select", "misc_RNA");
  });
});
