...
const DataTable = ({
  ...
  const insertRows = ({ above, numRows = 1, appendToBottom } = {}) => {
  ...
  const showContextMenu = (e, { idMap, selectedCells } = {}) => {
    let selectedRecords;
    if (isCellEditable) {
      const rowIds = {};
      ...
      selectedRecords = entities.filter(ent => rowIds[getIdOrCodeOrIndex(ent)]);
    } else {
  ...
  const getTableCellProps = (state, rowInfo, column) => {
