...
describe("MenuBar", () => {
  ...
  it(`component:()=>{} item in a menubar can be searched!`, () => {
    cy.contains(".bp5-button-text", "Help").click();
    cy.focused().type("melting");
    cy.get(
      `.bp5-menu-item:contains(Melting Temp of Selection) .bp5-icon-small-tick`
    );
    cy.focused().clear().type("component cmd example");
    cy.get(
      `.bp5-menu-item:contains(component cmd example):contains(File) .bp5-key-combo`
    );
  });
  it(`hiddenButSearchable text in a menubar can be searched!`, () => {
    cy.contains(".bp5-button-text", "Help").click();
    cy.focused().type("hidden but");
    cy.get(`.bp5-menu-item:contains(Cmd With Ticks)`);
  });
  it(`menubar can be searched!`, () => {
    ...
    cy.contains(
      ".tg-menu-search-suggestions .bp5-menu-item.bp5-disabled",
      "I'm disabled"
    ).should("exist");
    // sub menus should be accessible on hover!
    cy.focused().type("{selectall}other");
    cy.contains(".tg-menu-search-suggestions .bp5-menu-item", "Other").trigger(
      "mouseover"
    );
    cy.contains(".bp5-menu-item", "XXXXX").click({ force: true });

    const closeToasts = () => {
      cy.get(".bp5-toast .bp5-icon-cross").each(el => {
        el.click();
      });
    };
    closeToasts();
    ...
    cy.get(".tg-menu-search-suggestions .bp5-menu-item").should(
      "have.length",
      10
    );
    ...
    cy.contains(".tg-menu-search-suggestions .bp5-menu-item", "Long React")
      .find("input")
      .type("ha");
    ...
    cy.contains(".bp5-menu-item", "Don't Dismiss").click();
    cy.contains(".bp5-toast", "This menu's not going away any time soon");
    cy.get(`.tg-menu-bar-help-search input`).focus();

    cy.triggerFileCmd("{selectall}React", { noEnter: true, noOpen: true });
    cy.contains(".bp5-menu-item", "ReactText 9").click();
    cy.contains(".bp5-toast", "Fired ReactText!");

    //it can get the File > ReactText path correctly
    closeToasts();
    cy.triggerFileCmd("hel", { noEnter: true });
    cy.contains(".bp5-menu-item", "File > ReactText");

    //menu items with hideFromMenuSearch should not show up
    cy.triggerFileCmd("{selectall}About", { noEnter: true, noOpen: true });

    cy.contains(".bp5-menu-item-label", "Help").should("not.exist");

    //dynamic cmd created submenus should be visible
    cy.triggerFileCmd("{selectall}cmd", { noEnter: true, noOpen: true });
    cy.contains(".bp5-menu-item", "Cmd Submenu");
    cy.contains(".bp5-menu-item", "File > ReactText");
